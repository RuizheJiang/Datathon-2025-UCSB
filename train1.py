...
# Example Usage
model = PredModel_withConcat(103, 512, 5)
train_loss, train_acc, best_eval_accuracy = train(model, 1000, 128, 0.0001, matches, device='cuda:7')
print(best_eval_accuracy)
save_model(model, "predmodel_concat/final_model.pth")

plot_and_save(train_loss, train_acc, filename='concat.png')
# Load Model
# model = PredModel_withFeatures(103, 512, 10)
# model = load_model(model, "predmodel_concat/final_model.pth", device='cuda:0')

# Load Checkpoint
# optimizer = optim.Adam(model.parameters(), lr=0.00001)
# model, optimizer, epoch, best_acc = load_checkpoint(model, optimizer, "predmodel_concat/best_model.pth", device='cuda:0')
